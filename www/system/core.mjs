...
class AxmCore{
    async init(){
        this.pages = {}
        this.projects = [
          {domain: "api.axman.dk", name: "Løn"},
          {domain: "intapi.axman.dk", name: "Internt"}
        ]
        ...
        this.state = {
            ...
            query: queryArgs,
            project: queryArgs.project || this.projects[0].domain
        }
        ...
        if(this.isMobile()){
          ...
          on("changed-page", "core", () => document.getElementById("grid-container").classList.add("collapsed"))
        }
    }
    ...
    async changeProject(newProject){
      ...
      this.pushCurState();
      fire("changed-project", newProject)
    }
    ...
    async pushCurState(){
      ...
      if(this.state.project != this.projects[0].domain){
        query.project = this.state.project
      } else {
        delete query.project
      }

      if(Object.entries(query).length > 0){
        let q = Object.entries(query).map(([k, v]) => `${k}=${v}`).join("&")
        window.history.pushState(this.state, null, `${this.state.path}?${q}`);
      } else {
          window.history.pushState(this.state, null, this.state.path);
      }
    ...
    async render(){
        ...
        if(this.pages[this.state.path]){
            newPage = this.pages[this.state.path]
        } else {
            let pagePath = route(this.state.path)
        ...
        this.pages[this.state.path] = newPage
    }
